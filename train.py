...
class ImageSequence(keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        # x and y do not match in len - check this
        # y = [numpy.zeros((self.captcha_length, len(self.captcha_symbols)), dtype=numpy.uint8) for i in
        #     range(self.batch_size)]
        # y = numpy.zeros((self.batch_size, self.captcha_length, len(self.captcha_symbols)), dtype=numpy.uint8)

        # check what happens when the batch size is larger than the available files in the batch
        ...
        for i in range(min(self.batch_size, len(self.files))):
            ...
            try:
                ...
                print('Exception details ' + str(exc) +
                      ' Name of file= ' + random_image_file)
                continue
...
# check the output if the label of y matches the output of 37
